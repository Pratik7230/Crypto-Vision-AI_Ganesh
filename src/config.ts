...
export async function loadConfig(): Promise<Config> {
  ...
  const backendCanisterId = normalizeEnvValue(
  ...
  try {
    ...
    const resolvedCanisterId =
      normalizeEnvValue(config.backend_canister_id) ??
      backendCanisterId ??
      "aaaaa-aa";
    ...
    const fullConfig = {
      ...
      storage_gateway_url:
        process.env.NEXT_PUBLIC_STORAGE_GATEWAY_URL ?? "nogateway",
    ...
    const fallbackConfig = {
      ...
      backend_canister_id: backendCanisterId ?? "aaaaa-aa",
...
async function maybeLoadMockBackend(): Promise<backendInterface | null> {
  ...
  try {
    // Load an optional mock backend only when explicitly enabled.
    const mod = (await import("./mocks/backend")) as {
